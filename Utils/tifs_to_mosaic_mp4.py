...
input_filenames = ['/Data/analysis_2pRAM/VolumetricTest/Max30_500umdeep_1p2by1p2mm_3umppix_18p82Hz_250mW_00001_00001_Reshaped_plane0.tif',
...
for tif_i in range(len(input_filenames)):
    ...
    tif = scipy.signal.convolve(tif, np.ones(([rolling_average_frames,1,1])), mode='valid')
...
out = cv2.VideoWriter(output_filename, cv2.VideoWriter_fourcc(*'mp4v'), fps, size, False)
