...
# TURNED OFF SYSTEM-OOM. 
# It might be worth trying to increase the thresholds for when to kill processes (currently 50% and 20secs, although the memory )
# https://askubuntu.com/questions/1404888/how-do-i-disable-the-systemd-oom-process-killer-in-ubuntu-22-04
#
# TURNED OFF WITH:
# $ systemctl disable --now systemd-oomd
# $ systemctl mask systemd-oomd
# IT CAN BE TURNED BACK ON WIT:
...
# Directories
raw_data_dirs = [r'/home/freiwald/Data/analysis_2pRAM/Coconut/20230106d/SP_200umdeep_1p2mm_3umppix_18p82Hz_40mW/'] # Must be a list with 1 or more dirs
raw_data_dirs = [r'/home/freiwald/Data/analysis_2pRAM/Yartifacts/']
fname_must_contain = 'motion' #something you want to specify and that the desired filenames should contain
fname_must_NOT_contain = 'some_random_stuff' #if not needed, leave something you know it is not in the filename

# Parameters output volume
save_as_volume_or_planes = 'planes' # You can either save individual planes in subfolders, or a whole 4D volume
make_nonan_volume = True # Also affects planes if lateral_aligned_planes==True or identical_mroi_overlaps_across_planes==False
lateral_align_planes = False
identical_mroi_overlaps_across_planes = True
add_1000_for_nonegative_volume = True
save_output = True # should be true unless testing...
concatenate_all_h5_to_tif = False

# Do not change this... 
if not lateral_align_planes and identical_mroi_overlaps_across_planes:
    initialize_volume_with_nans = False
    convert_volume_float32_to_int16 = True
    make_nonan_volume = False # It is going to be no-nan by definition, no need to check for it
elif make_nonan_volume:
...
seams_overlap = 0#'calculate' # Should be either 'calculate', an integer, or a list of length n_planes
...
json_filename = f"{raw_data_dirs[0]}log_{date_string}.json"
...
for current_pipeline_step in pipeline_steps:
    ...
    for i_file in list_files_for_reconstruction:
        ...
        if json_logging: json_logger.debug(json.dumps({'debug_message':'Loading file (expect warning if multi-file recording)'}))
        ...
        if current_pipeline_step == 'make_template':
            if seams_overlap == 'calculate':
                ...
                overlaps_planes_seams_scores = np.zeros((n_planes, n_mrois-1, max_seam_overlap-min_seam_overlap))  # We will avoid i_overlaps = 0
                
                for i_plane in range(n_planes):
                    for i_seam in range(n_mrois-1):
                        for i_overlaps in range(min_seam_overlap,max_seam_overlap):
                            strip_left  = planes_mrois[i_plane, i_seam  ][0, -n_ignored_pixels_sides-i_overlaps:-n_ignored_pixels_sides]
                            strip_right = planes_mrois[i_plane, i_seam+1][0, n_ignored_pixels_sides:i_overlaps +n_ignored_pixels_sides]
                            subtract_left_right = abs(strip_left - strip_right)
                            overlaps_planes_seams_scores[i_plane, i_seam, i_overlaps-min_seam_overlap] = np.mean(subtract_left_right)
                                
                overlaps_planes_scores = np.mean(overlaps_planes_seams_scores, axis = (1))
                overlaps_planes = []
                for i_plane in range(n_planes):
                    overlaps_planes.append(int(np.argmin(overlaps_planes_scores[i_plane])+ min_seam_overlap + 2 * n_ignored_pixels_sides))
                ...
                if alignment_plot_checks:
                    ...
                    for i_plane in range(n_planes):
                        plt.plot(range(min_seam_overlap,max_seam_overlap), overlaps_planes_scores[i_plane])
                    ...
                    for i_plane in range(n_planes):
                        for shift in range(-2, 3):
                            ...
                            plt.show()
                if identical_mroi_overlaps_across_planes:
                    overlaps_planes = [int(round(np.mean(overlaps_planes)))] * n_planes
                    
            elif type(seams_overlap) is int:
        ...
        if current_pipeline_step == 'make_template' or not lateral_align_planes:
        ...
        if add_1000_for_nonegative_volume:
            if json_logging: json_logger.debug(json.dumps({'debug_message':'Adding 1000 to make positive'}))
            volume += 1000
            
        #%% Convert volume from float to int
        if convert_volume_float32_to_int16:
            if volume.dtype != np.int16:
                if json_logging: json_logger.debug(json.dumps({'debug_message':'Transforming volume to int16'}))
        ...
        if current_pipeline_step == 'reconstruct_all':
            if save_output:
                ...
                save_dir = os.path.dirname(path_input_file) + '/Reconstructed/' 
                if save_as_volume_or_planes == 'volume':
                    ...
                    for i_plane in range(n_planes):
                        ...
                        if i_file == list_files_for_reconstruction[-1] and concatenate_all_h5_to_tif:
                            files_to_concatenate = sorted(glob.glob(save_dir_this_plane + '*Aligned.h5'))
                            data_to_concatenate = []
                ...
                if json_logging: json_logger.debug(json.dumps({'debug_message':'File(s) saved'}))
                
                #%% Save mean frame png
                if save_meanf_png:
                    if not meanf_png_only_first_file or i_file == 0:
                ...
                if save_mp4:
                    ...
                    canvas_video = np.zeros((volume[:use_until_frame_n].shape[0] - rolling_average_frames + 1, 
                                       ...
                                       (volume.shape[2] + gaps_columns) * columns - gaps_columns),
                                       dtype = np.uint8)
                    if not video_only_first_file or i_file == 0:
                        for i_plane in range(n_planes):
                        ...
                        input_filename = os.path.basename(path_input_file) 
                        output_filename_video = save_dir + input_filename[:-4] + '_RollingAvg' + str(rolling_average_frames) + 'Frames_Speed' + str(video_play_speed) + 'x.mp4'
