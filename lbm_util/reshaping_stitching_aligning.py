...
# params[
#     'seams_overlap'] = 'calculate'  # Should be either 'calculate', an integer, or a list of integers with length=n_planes
# params['save_output'] = True
# params[
#     'make_nonan_volume'] = True  # Whether to trim the edges so the output does not have nans. Also affects output as planes if lateral_aligned_planes==True (to compensate for X-Y shifts of MAxiMuM) or params['identical_mroi_overlaps_across_planes']==False (if seams from different planes are merged differently, then some planes will end up being larger than others)
...
if params['save_output']:
    params[
        'save_as_volume_or_planes'] = 'planes'  # 'planes' will save individual planes in subfolders -- 'volume' will save a whole 4D hdf5 volume
    if params['save_as_volume_or_planes'] == 'planes':
        params[
            'concatenate_all_h5_to_tif'] = False  # If True, it will take all the time-chunked h5 files, concatenate, and save them as a single .tif
if params['seams_overlap'] == 'calculate':
    params[
        'n_ignored_pixels_sides'] = 5  # Useful if there is a delay or incorrect phase for when the EOM turns the laser on/off at the start/end of a resonant-scanner line
...
for current_pipeline_step in pipeline_steps:
    ...
    for i_file in list_files_for_reconstruction:
        ...
        print('---------------------------------------------------')
        if params['json_logging']:
            json_logger.debug(json.dumps({'debug_message': 'Started working on: ' + str(path_input_file)}))
        ...
        if i_file == 0:
            ...
            if params['json_logging']: json_logger.debug(
        ...
        if params['json_logging']: json_logger.debug(
            json.dumps({'debug_message': 'Loading file (expect warning if multi-file recording)'}))
        ...
        if current_pipeline_step == 'make_template':
            ...
            assert np.product(
                np.isclose(pixel_sizes[:, 1] - psize_y, 0)), "Y-pixels resolution not uniform across MROIs"
            assert np.product(
                np.isclose(pixel_sizes[:, 0] - psize_x, 0)), "X-pixels resolution not uniform across MROIs"
            ...
            for i_xy in range(2):
                for i_mroi in range(n_mrois):
                    closest_xy_pix = np.argmin(
                        np.abs(reconstructed_xy_ranges_si[i_xy] - top_left_corners_si[i_mroi, i_xy]))
                    ...
                    if not np.isclose(closest_xy_si, top_left_corners_si[i_mroi, i_xy]):
                        if params['json_logging']: json_logger.debug(json.dumps({
        ...
        if current_pipeline_step == 'make_template':
            if params['seams_overlap'] == 'calculate':
                ...
                for i_mroi in range(n_mrois - 1):
                    if top_left_corners_pix[i_mroi][0] + sizes_mrois_pix[i_mroi][0] != top_left_corners_pix[i_mroi + 1][
                        0]:
                    ...
                    for i_seam in range(n_mrois - 1):
                        for i_overlaps in range(params['min_seam_overlap'], params['max_seam_overlap']):
                            ...
                            overlaps_planes_seams_scores[
                                i_plane, i_seam, i_overlaps - params['min_seam_overlap']] = np.mean(subtract_left_right)
        ...
        if params['json_logging']: json_logger.debug(
            json.dumps({'debug_message': 'Creating volume of shape: ' + str([n_f, n_x, n_y, n_z]) + ' (f,x,y,z)'}))
        ...
        if params['json_logging']: json_logger.debug(
            json.dumps({'debug_message': 'Merging MROIs and placing them into the volume'}))
        for i_plane in range(n_planes):
            ...
            for i_mroi in range(n_mrois):
                ...
                if i_mroi == 0:
                    ...
                    x_end_canvas = x_start_canvas + sizes_mrois_pix[i_mroi][0] - int(
                        np.trunc(overlap_seams_this_plane / 2))
                ...
                plane_canvas[:, x_start_canvas:x_end_canvas, y_start_canvas:y_end_canvas] = planes_mrois[
                                                                                                i_plane, i_mroi][:,
                                                                                            x_start_mroi:x_end_mroi]
        ...
        if params['make_nonan_volume']:
            ...
            if params['json_logging']: json_logger.debug(
                json.dumps({'debug_message': 'Shape of trimmed nonan volume: ' + str(volume.shape)}))
                ...
                if params['save_meanf_png']:
                    if not params['meanf_png_only_first_file'] or i_file == 0:
                        ...
                        canvas_png = np.zeros(((volume.shape[1] + params['gaps_rows']) * rows - params['gaps_rows'],
                                               (volume.shape[2] + params['gaps_columns']) * columns - params[
                                                   'gaps_columns']),
                                              dtype=np.uint8)
                        ...
                        for i_plane in range(n_planes):
                            ...
                            plane_for_png = skimage.exposure.rescale_intensity(plane_for_png,
                                                                               in_range=(pct_low, pct_high))
                ...
                if params['save_mp4']:
                    if not params['video_only_first_file'] or i_file == 0:
                        ...
                        canvas_video = np.zeros(
                            (volume[:use_until_frame_n].shape[0] - params['rolling_average_frames'] + 1,
                        ...
                        for i_plane in range(n_planes):
                            ...
                            plane_for_video = scipy.signal.convolve(plane_for_video,
                                                                    np.ones(([params['rolling_average_frames'], 1, 1])),
                                                                    mode='valid')
                            # Apply percentile-dynamic-range
                            pct_low, pct_high = np.nanpercentile(plane_for_video, params['intensity_percentiles'])
                            plane_for_video = skimage.exposure.rescale_intensity(plane_for_video,
                                                                                 in_range=(pct_low, pct_high))
                            # Normalize to [0,1]
                        ...
                        input_filename = os.path.basename(path_input_file)
                        output_filename_video = save_dir + input_filename[:-4] + '_RollingAvg' + str(
                            params['rolling_average_frames']) + 'Frames_Speed' + str(
                            params['video_play_speed']) + 'x.mp4'
                        out = cv2.VideoWriter(output_filename_video, cv2.VideoWriter_fourcc(*'mp4v'), fps,
                                              size_frame_video, False)
        ...
        toc = time.time()
        if params['json_logging']: json_logger.debug(
            json.dumps({'debug_message': 'File processed and outputs saved. Time elapsed: ' + str(toc - tic)}))
        del volume
